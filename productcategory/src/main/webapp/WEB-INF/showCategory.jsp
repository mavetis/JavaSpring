...
<body>
	<div class="container text-center">
		<h2 class="title text-center pt-5 fw-lighter">Category</h2>
		<div class="row">
		<h4 class="mb-5" style="font-family: 'Marmelad', sans-serif;"><c:out value="${category.name}"/></h4>
			<div class="col d-flex justify-content-center">
				<div class="card text-center border-0 shadow-lg p-3 mb-5 bg-body rounded-pilled bg-transparent" >
					<div class="card=body">
					<h5 class="fw-lighter">Products:</h5>
						<c:forEach items="${category.products}" var="pro">
							<p style="font-family: 'Marmelad', sans-serif;"><c:out value="${pro.name}"/></p>
						</c:forEach>
					<form:form action="/attachProduct" method="post" modelAttribute="proCat">
						<p class="fw-lighter">
							<form:label path="product">Add Product:</form:label>
							<form:select path="product" class="border-0 bg-transparent text-center fw-lighter">
								<c:forEach items="${products}" var="pro">
									<form:option value="${pro.id}"><c:out value="${pro.name}"/></form:option>
								</c:forEach>
							</form:select>
						</p>
